...
class TestDry(unittest.TestCase):
    ...
    def testCorrectEmtpyDwi(self):
        ...
        model = dann.train_model(bval1s)
        with self.assertRaises(ValueError):
            dann.correct_fwe(None, model, bval1s, testingdir)

    def testCorrectEmptyModel(self):
        dann = Dry()
        with self.assertRaises(ValueError):
            dann.correct_fwe(dwis1s, None, bval1s, testingdir)

    def testCorrectEmptyBval(self):
        dann = Dry()
        model = dann.train_model(bval1s)
        with self.assertRaises(ValueError):
            dann.correct_fwe(dwis1s, model, None, testingdir)

    def testCorrectDifferentBval(self):
        dann = Dry()
        model = dann.train_model(bval1s)
        with self.assertRaises(ValueError):
            dann.correct_fwe(dwis1s, model, bval2s, testingdir)

    def testCorrectEmptyOutputFolder(self):
        dann = Dry()
        model = dann.train_model(bval1s)
        dann.correct_fwe(dwis1s, model, bval1s)
    ...
    def testCorrectOneShellOk(self):
        ...
        dann.correct_fwe(dwis1s, model1s, bval1s, testingdir)
    ...
    def testCorrectTwoShellOk(self):
        ...
        dann.correct_fwe(dwis2s, model2s, bval2s, testingdir)
